...
public class SensorTransmissionCoder {
    ...
    public static class SensorData implements Parcelable {
        ...
        private void initializeSensorMessage(int deviceType, int sensorType, float[] value, long timestamp) {
            ...
            // Deep copy the value array, first item is completely necessary
            this.value[0] = value[0];

            // Copy optional values if they were received in other case use 0 as null
            this.value[1] = value.length > 1 ? value[1] : 0;
            this.value[2] = value.length > 2 ? value[2] : 0;
